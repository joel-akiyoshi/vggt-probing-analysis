...
from probing_experiment.data_loading.correspondences import get_all_image_pair_correspondences, print_correspondence_summary
...
images = load_images_txt(experiment_root / "data/highres_train/courtyard/dslr_calibration_undistorted/images.txt")
pair_corrs = get_all_image_pair_correspondences(images, min_matches=100)
# print_correspondence_summary(images, pair_corrs)

# one pair test
scene_root = experiment_root / "data/highres_train/courtyard/images"
...
# aggregate into training (11 scenes) and testing (2 scenes), load each into respective Dataset
    ...
    # With X_train + GT correspondences: feed pred_f = MLP(X_train), backprop with loss(pred_f, GT_corr), iterate for 30 epochs
